...
def main():
    ...
    # weight for sequence
    parser.add_argument('--seqWeight', type=float, default=0,
                        help='Weight to be used in sequence')
    # weight function
    parser.add_argument('--seqWeight_func', type=str, default='add', 
                        choices = ['add', 'multi'],
                        help='Weight function to be used')
    ...
    parser.add_argument('--load', type=str, default='pretrained/best.tar',
...
def train(args):
    ...
    args.freq_validation = np.clip(args.freq_validation, 1, args.num_epochs)
    validation_epoch_list = list(np.arange(0, args.num_epochs, args.freq_validation))
    ...
    for epoch in range(args.num_epochs):
        ...
        for batch in range(dataloader.num_batches):
            ...
            if dataset_pointer_ins_grid != dataloader.dataset_pointer or epoch != 0:
                num_batch = 0
            ...
            for sequence in range(dataloader.batch_size):
                ...
                # Compute loss
                loss = Gaussian2DLikelihood(outputs, x_seq, PedsList_seq, lookup_seq, args.seqWeight, args.seqWeight_func)
        ...
        if dataloader.valid_num_batches > 0:
            ...
            for batch in range(dataloader.valid_num_batches):
                ...
                for sequence in range(dataloader.batch_size):
                    ...
                    # Compute loss
                    loss = Gaussian2DLikelihood(outputs, x_seq[1:], PedsList_seq[1:], lookup_seq, args.seqWeight, args.seqWeight_func)
