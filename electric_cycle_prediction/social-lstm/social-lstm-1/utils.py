...
class DataLoader():
    ...
    def __init__(self, f_prefix, batch_size=5, seq_length=20, num_of_validation=0, forcePreProcess=False, infer=False,
        ...
        self.dataset_dimensions = {'elec': [1920, 1080]}
    ...
    def frame_preprocess(self, data_dirs, data_file, validation_set=False):
        ...
        for directory in data_dirs:
            ...
            if self.infer is False:
                ...
                # hly
                ids = []
                for i in range(0, len(df), self.seq_length):  ##每隔20行取数据
                    ids.append(i)
                self.target_ids = np.array(df.iloc[ids]['ped_id'])
                print(self.target_ids.shape)
            ...
            for ind, frame in enumerate(frameList):
                ...
                # hly
                # delete duplicates
                pedsInFrame = np.unique(pedsInFrame, axis=1)
                if self.infer and self.additional_validation == False:
                    peds_dict = dict()
                    for idx in range(pedsInFrame.shape[1]):
                        cur_id = pedsInFrame[1, idx]
                        cur_data = pedsInFrame[:, idx].copy()
                        cur_data[1] = 0
                        if cur_id not in peds_dict.keys():
                            peds_dict[cur_id] = cur_data
                        elif ~np.isnan(cur_data[2]) and cur_id in peds_dict.keys():
                            peds_dict[cur_id] = cur_data
                    peds = []
                    for key, value in peds_dict.items():
                        value[1] = key
                        peds.append(value.reshape(-1, 1))
                    peds = np.hstack(peds)
                    pedsInFrame = peds
                #     p_mask = np.logical_and(~np.isnan(pedsInFrame[2, :]), pedsInFrame[1, :] != self.target_ids[ind])
                #     pedsInFrame = pedsInFrame[:, p_mask]
