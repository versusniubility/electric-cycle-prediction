...
def get_mean_error(ret_nodes, nodes, assumedNodesPresent, trueNodesPresent, using_cuda, look_up):
    ...
    for tstep in range(pred_length):
        ...
        for nodeID in assumedNodesPresent[tstep]:
            ...
            maxerror = max(maxerror, torch.norm(pred_pos - true_pos, p=2))
            error[tstep] += torch.norm(pred_pos - true_pos, p=2)
            counter += 1

        if counter != 0:
            error[tstep] = error[tstep] / counter
            print(f'tstep : {tstep}, mean_error : {error[tstep]}, max_error : {maxerror}')
...
def Gaussian2DLikelihood(outputs, targets, nodesPresent, look_up, weight=0, weight_func='add'):
    '''
    params:
    ...
    for framenum in range(seq_length):
        ...
        for nodeID in nodeIDs:
            nodeID = look_up[nodeID]
            if weight_func == 'add':
                loss = loss + (1.0 + float(weight) * framenum) * result[framenum, nodeID]
            elif weight_func == 'multi':
                loss = loss + pow(1.0 + float(weight), framenum) * result[framenum, nodeID]
            counter = counter + 1
...
def revert_seq(x_seq, PedsList_seq, lookup_seq, first_values_dict):
    ...
    for ind, frame in enumerate(x_seq):
        for ped in PedsList_seq[ind]:
            # print(frame[lookup_seq[ped], 0:2].device)
            # print(first_values_dict[ped][0:2].device)
    ...
    with torch.no_grad():
        ...
        for tstep in range(args.seq_length - 1):
            ...
            if not args.use_cuda:
                out_, hidden_states, cell_states = net(x_seq[tstep].view(1, numx_seq, 2), [grid[tstep].cpu()], hidden_states,
